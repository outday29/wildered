...
def test_update_with_signature(parser: BaseDirectiveParser):
    ...
    source = ASTSourceCode.from_file("./tests/example_scripts/basic.py")
    entity_list = parser.parse(source=source)
    new_function_code = read_file("./tests/example_scripts/new_function_sign.py")
    new_class_code = read_file("./tests/example_scripts/new_class_sign.py")
    new_method_code = read_file("./tests/example_scripts/new_method_sign.py")
    ...
    # source.save("./tests/expected_scripts/updated_signature.py")
    # Should never change the original script when filename is passed explicitly
    assert read_file("./tests/example_scripts/basic.py") == orig
    with tempfile.NamedTemporaryFile() as f:
        source.save(f.name)
        assert read_file(f.name) == read_file("./tests/expected_scripts/updated_signature.py")
...
def test_update_file(parser: BaseDirectiveParser):
    ...
    source = ASTSourceCode.from_file("./tests/example_scripts/basic.py")
    new_file = read_file("./tests/example_scripts/new_file.py")
    entity_list = parser.parse(source=source)
    entity_list[0].update(new_file)
    # source.save("./tests/expected_scripts/updated_script.py")
    
    assert read_file("./tests/example_scripts/basic.py") == orig
    with tempfile.NamedTemporaryFile() as f:
        source.save(f.name)
        assert read_file(f.name) == read_file("./tests/expected_scripts/updated_script.py")